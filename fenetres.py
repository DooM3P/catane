...
class board:
    def __init__(self):
        ...
        self.cf1.grid()
        for coord in list(jeu.ile.hexacoord.keys()):  # trace des hexagones
    ...
    def drawNumbers(self, _numde, _picture, _tags):
        for i in list(jeu.ile.hexacoord.keys()):
    ...
    def mouse(self, event):  # fonction qui selectionne le sommet
        ...
        x = (x - 200) / 1.75
        y = (y - 200) / 1.75
        for i, j in list(jeu.ile.sommetscoord.keys()):
...
class rogue:
    def __init__(self, _gestionnaireFenetres):
        ...
        self.label3 = Label(self.f2, bg="White", text="Choisissez le nouvel emplacement du brigand puis cliquez",
                            width=45)
        self.label3.grid()
    ...
    def mouse2(self, event):
        x, y = self.gestionnaireFenetres.fenetreCourante.cf1.canvasx(
            event.x), self.gestionnaireFenetres.fenetreCourante.cf1.canvasy(event.y)
        x = (x - 200) / 1.75
        y = (y - 200) / 1.75
        for i, j in list(jeu.ile.hexacoord.keys()):
        ...
        self.f2.destroy()
        self.gestionnaireFenetres.fenetreCourante.cf1.bind('<Button-1>',
                                                           self.gestionnaireFenetres.fenetreCourante.mouse)
...
class score:  # ce panneau affiche principalement des infos sur les couts de constructions et les sommets selectionnes
    def __init__(self, _gestionnaireFenetres):
        ...
        self.label2 = Label(self.f3, bg="White", textvariable=self.gestionnaireFenetres.fenetreCourante.num, width=25,
                            anchor=W)
    ...
    def ok(self):
        if not self.v.get() == "":
            ...
            if jeu.plyrcount < jeu.njoueurs - jeu.ncpu:
                ...
                for i in range(0,
                               jeu.ncpu):  # une fois tous les joueurs humains crees, on cree les ordinateurs, si necessaire
                    Plyr1 = Joueur(jeu.find(jeu.listjoueurs), 1, jeu.colors[
                        0])  # la fonction find "trouve" le numero de de du joueur en relancant si necessaire
                    jeu.plyrcount = jeu.plyrcount + 1
                    del jeu.colors[0]
                    jeu.listjoueurs[Plyr1.color] = Plyr1
...
class score2:  # ici ressources mises a jour en temps reel, points de temps en temps
    def __init__(self, _gestionnaireFenetres):
        ...
        for i in range(0, len(jeu.listjoueurs)):
            self.f3.bouton1 = Label(self.f3, bg="White", text=jeu.findplyr(jeu.listjoueurs, jeu.order[i]), width=5,
                                    anchor=N)
            self.f3.bouton1.grid(column=0, row=i + 1)
            self.f3.bouton1 = Label(self.f3, bg="White",
                                    textvariable=jeu.listjoueurs[jeu.findplyr(jeu.listjoueurs, jeu.order[i])].pts,
                                    width=5, anchor=N)
            self.f3.bouton1.grid(column=1, row=i + 1)
            self.f3.bouton1 = Label(self.f3, bg="White",
                                    textvariable=jeu.listjoueurs[jeu.findplyr(jeu.listjoueurs, jeu.order[i])].bois,
                                    width=5, anchor=N)
            self.f3.bouton1.grid(column=2, row=i + 1)
            self.f3.bouton1 = Label(self.f3, bg="White",
                                    textvariable=jeu.listjoueurs[jeu.findplyr(jeu.listjoueurs, jeu.order[i])].argile,
                                    width=5, anchor=N)
            self.f3.bouton1.grid(column=3, row=i + 1)
            self.f3.bouton1 = Label(self.f3, bg="White",
                                    textvariable=jeu.listjoueurs[jeu.findplyr(jeu.listjoueurs, jeu.order[i])].ble,
                                    width=5, anchor=N)
            self.f3.bouton1.grid(column=4, row=i + 1)
            self.f3.bouton1 = Label(self.f3, bg="White",
                                    textvariable=jeu.listjoueurs[jeu.findplyr(jeu.listjoueurs, jeu.order[i])].laine,
                                    width=5, anchor=N)
            self.f3.bouton1.grid(column=5, row=i + 1)
            self.f3.bouton1 = Label(self.f3, bg="White",
                                    textvariable=jeu.listjoueurs[jeu.findplyr(jeu.listjoueurs, jeu.order[i])].minerai,
                                    width=7, anchor=N)
            self.f3.bouton1.grid(column=6, row=i + 1)
            self.f3.bouton1 = Label(self.f3, bg="White",
                                    text=jeu.listjoueurs[jeu.findplyr(jeu.listjoueurs, jeu.order[i])].numplyr, width=9,
                                    anchor=N)
...
class build:  # on pose les deux premieres colonies gratuites
    def __init__(self, _gestionnaireFenetres):
        ...
        if jeu.plyrcount < jeu.njoueurs:
            jeu.currentcolor = jeu.findplyr(jeu.listjoueurs, jeu.order[
                jeu.plyrcount])  # la fonction jeu.findplyr permet un ordre conforme aux des
            self.f2.label1 = Label(self.f2,
                                   text=("Joueur " + str(jeu.currentcolor) + ", a vous de poser votre colonie"))
            self.f2.label1.grid()
    ...
    def ok(self, event):
        ...
        if jeu.currentcolor == 'Vert':
            ...
            label = Label(image=photo)
            label.image = photo
        if jeu.ile.sommets[self.gestionnaireFenetres.fenetreCourante.num.get()].constr == (0, 0) and jeu.ile.somtest(
                self.gestionnaireFenetres.fenetreCourante.num.get(), jeu.ile.sommets[
                    self.gestionnaireFenetres.fenetreCourante.num.get()].listsuccess) == 0:  # si la case n'est pas construite et le sommet a plus de deux aretes de distance, on peut poser
            jeu.ile.sommets[self.gestionnaireFenetres.fenetreCourante.num.get()].constr = 1, jeu.currentcolor
            jeu.listjoueurs[jeu.currentcolor].villages[self.gestionnaireFenetres.fenetreCourante.num.get()] = \
            jeu.ile.sommets[self.gestionnaireFenetres.fenetreCourante.num.get()]
            item = self.gestionnaireFenetres.fenetreCourante.cf1.create_image(
                (jeu.ile.sommets[self.gestionnaireFenetres.fenetreCourante.num.get()].x * 1.75) + 200,
                (jeu.ile.sommets[self.gestionnaireFenetres.fenetreCourante.num.get()].y * 1.75) + 200,
                image=label.image, tags="image")
            self.f2.destroy()
...
class road:  # ici, pour poser la route, on ne demande que l'arrivee au joueur, le depart etant forcement la colonie tout juste posee
    def __init__(self, _gestionnaireFenetres):
        ...
        self.f2.label1 = Label(self.f2,
                               text=("Joueur " + str(jeu.currentcolor) + ", posez maintenant l'arrivee de votre route"))
        self.f2.label1.grid()
        ...
        if self.gestionnaireFenetres.fenetreCourante.num.get() != jeu.route1.coord[0]:  # arrivee differente du depart
            for i in jeu.ile.sommets[self.gestionnaireFenetres.fenetreCourante.num.get()].listsuccess:
                if i.num == jeu.route1.coord[0]:  # il faut que les sommets soient voisins !
                    ...
                    self.gestionnaireFenetres.fenetreCourante.cf1.create_line(
                        jeu.ile.sommets[jeu.route1.coord[0]].x * 1.75 + 200,
                        jeu.ile.sommets[jeu.route1.coord[0]].y * 1.75 + 200,
                        jeu.ile.sommets[jeu.route1.coord[1]].x * 1.75 + 200,
                        jeu.ile.sommets[jeu.route1.coord[1]].y * 1.75 + 200, width=4,
                        fill=jeu.transcolor(jeu.currentcolor))
...
class game1:  # Juste le lancer de de au debut de chaque tour
    def __init__(self, _gestionnaireFenetres):
        ...
        if jeu.plyrcount < jeu.njoueurs:
            jeu.currentcolor = jeu.findplyr(jeu.listjoueurs, jeu.order[
                jeu.plyrcount])  # La couleur du joueur qui joue, determinee par l'ordre des des
...
class game2:  # les differents boutons d'action du joueurs (poser une route, une colonie, ville faire du commerce, acheter une carte)
    ...
    def colonie(self):
        if jeu.listjoueurs[jeu.currentcolor].bois.get() > 0 and jeu.listjoueurs[jeu.currentcolor].argile.get() > 0 and \
                jeu.listjoueurs[jeu.currentcolor].ble.get() > 0 and jeu.listjoueurs[jeu.currentcolor].laine.get() > 0:
            self.f2.destroy()
    ...
    def carte(self):
        if jeu.listjoueurs[jeu.currentcolor].ble.get() > 0 and jeu.listjoueurs[jeu.currentcolor].laine.get() > 0 and \
                jeu.listjoueurs[jeu.currentcolor].minerai.get() > 0:
            if len(jeu.cards) > 0:
...
class road2:  # plus compliquee que road1 car deux extremites a definir et il faut veiller a respecter plus de contraintes
    def __init__(self, _gestionnaireFenetres):
        ...
        self.f2 = Frame()
        self.f2.grid(column=1, row=1)
        self.f2.label1 = Label(self.f2, text=("Joueur " + str(
            jeu.currentcolor) + ", cliquez sur le depart \n puis l'arrivee de votre route, et appuyez sur OK"))
        self.f2.label1.grid()
        self.f2.bouton2 = Button(self.f2, text="OK", command=(lambda: self.ok2("blank")))
        self.f2.bouton2.grid()
        self.f2.bind_all('<Return>', self.ok2)
    ...
    def ok2(self, event):
        if self.gestionnaireFenetres.fenetreCourante.num.get() != self.gestionnaireFenetres.fenetreCourante.numans.get() and jeu.routeverif(
                self.gestionnaireFenetres.fenetreCourante.num.get(),
                self.gestionnaireFenetres.fenetreCourante.numans.get(),
                jeu) == 1:  # condition : depart et arrivee differentes ; sommets voisins
            if (self.gestionnaireFenetres.fenetreCourante.num.get(),
                self.gestionnaireFenetres.fenetreCourante.numans.get()) not in jeu.ile.routes and (
            self.gestionnaireFenetres.fenetreCourante.numans.get(),
            self.gestionnaireFenetres.fenetreCourante.num.get()) not in jeu.ile.routes:  # condition : la route ne doit pas deja exister ! (coordonnees testees dans les deux sens)
                if jeu.ile.routetest(self.gestionnaireFenetres.fenetreCourante.num.get(),
                                     self.gestionnaireFenetres.fenetreCourante.numans.get(), jeu.currentcolor,
                                     jeu) == 1:  # on verifie la presence d'une colonie ou route voisine de meme couleur
                    jeu.route1 = Route([self.gestionnaireFenetres.fenetreCourante.num.get(),
                                        self.gestionnaireFenetres.fenetreCourante.numans.get()], jeu.currentcolor)
                    self.gestionnaireFenetres.fenetreCourante.cf1.create_line(
                        jeu.ile.sommets[jeu.route1.coord[0]].x * 1.75 + 200,
                        jeu.ile.sommets[jeu.route1.coord[0]].y * 1.75 + 200,
                        ...
                        fill=jeu.transcolor(jeu.currentcolor))
                    jeu.ile.routes[(
                    jeu.route1.coord[0], jeu.route1.coord[1])] = jeu.route1  # on met la route dans la liste des routes
                    self.gestionnaireFenetres.fenetreCourante.cf1.lift("image")
                    jeu.listjoueurs[jeu.currentcolor].bois.set(
                        jeu.listjoueurs[jeu.currentcolor].bois.get() - 1)  # on fait "payer" les ressources
                    jeu.listjoueurs[jeu.currentcolor].argile.set(jeu.listjoueurs[jeu.currentcolor].argile.get() - 1)
                    self.f2.destroy()
                    self.gestionnaireFenetres.loadFenetreSuivante("game2")
                else:
                    print(("Il n'y a pas de route " + str(jeu.currentcolor) + " a cote"))
            else:
                print("Il y a deja une route ici")
        else:
            print("Route mal mise")
...
class colonie:
    def __init__(self, _gestionnaireFenetres):
        ...
        self.f2 = Frame()
        self.f2.grid(column=1, row=1)
        self.f2.label1 = Label(self.f2, text=(
                    "Joueur " + str(jeu.currentcolor) + ", cliquez sur le sommet \n puis appuyez sur OK (ou Enter)"))
        self.f2.label1.grid()
        self.f2.bouton2 = Button(self.f2, text="OK", command=(lambda: self.ok2("blank")))
        self.f2.bouton2.grid()
        self.f2.bind_all('<Return>', self.ok2)
    ...
    def ok2(self, event):
        ...
        if jeu.ile.sommets[self.gestionnaireFenetres.fenetreCourante.num.get()].constr == (0, 0) and jeu.ile.somtest(
                self.gestionnaireFenetres.fenetreCourante.num.get(), jeu.ile.sommets[
                    self.gestionnaireFenetres.fenetreCourante.num.get()].listsuccess) == 0:  # pas de colonie adjacente ?
            if jeu.ile.somtest2(self.gestionnaireFenetres.fenetreCourante.num.get(), jeu.currentcolor,
                                jeu) == 1:  # y a t-il une route voisine de la bonne couleur ?
                jeu.ile.sommets[self.gestionnaireFenetres.fenetreCourante.num.get()].constr = 1, jeu.currentcolor
                jeu.listjoueurs[jeu.currentcolor].villages[self.gestionnaireFenetres.fenetreCourante.num.get()] = \
                jeu.ile.sommets[self.gestionnaireFenetres.fenetreCourante.num.get()]
                item = self.gestionnaireFenetres.fenetreCourante.cf1.create_image(
...
class ville:
    ...
    def ok2(self, event):
        ...
        if jeu.currentcolor == 'Vert':
            ...
            label = Label(image=photo)
            label.image = photo
        if jeu.ile.sommets[self.gestionnaireFenetres.fenetreCourante.num.get()].constr == (
        1, jeu.currentcolor):  # il faut qu'il y ait deja une ville de meme couleur
...
class trade:
    def __init__(self, _root):
        ...
        self.fenetretop.f6.label = Label(self.fenetretop, text="Entrez votre offre, " + str(
    ...
    def rouge(self):  # Ici, j'aurais vraiment pu mettre une fonction pour eviter ces repetitions...
        ...
        if "Rouge" in jeu.listjoueurs:
            if jeu.listjoueurs["Rouge"].bois.get() >= int(self.fenetretop.f6.bois.get()) and jeu.listjoueurs[
                "Rouge"].argile.get() >= int(self.fenetretop.f6.argile.get()) and jeu.listjoueurs[
                "Rouge"].ble.get() >= int(self.fenetretop.f6.ble.get()) and jeu.listjoueurs["Rouge"].laine.get() >= int(
                    self.fenetretop.f6.laine.get()) and jeu.listjoueurs["Rouge"].minerai.get() >= int(
                    self.fenetretop.f6.minerai.get()):
                jeu.listjoueurs["Rouge"].bois.set(jeu.listjoueurs["Rouge"].bois.get() - int(
                    self.fenetretop.f6.bois.get()))  # on effectue les echanges de ressources
                jeu.listjoueurs["Rouge"].argile.set(
                    jeu.listjoueurs["Rouge"].argile.get() - int(self.fenetretop.f6.argile.get()))
                jeu.listjoueurs["Rouge"].ble.set(jeu.listjoueurs["Rouge"].ble.get() - int(self.fenetretop.f6.ble.get()))
                jeu.listjoueurs["Rouge"].laine.set(
                    jeu.listjoueurs["Rouge"].laine.get() - int(self.fenetretop.f6.laine.get()))
                jeu.listjoueurs["Rouge"].minerai.set(
                    jeu.listjoueurs["Rouge"].minerai.get() - int(self.fenetretop.f6.minerai.get()))
                jeu.listjoueurs[jeu.currentcolor].bois.set(
                    jeu.listjoueurs[jeu.currentcolor].bois.get() + int(self.fenetretop.f6.bois.get()))
                jeu.listjoueurs[jeu.currentcolor].argile.set(
                    jeu.listjoueurs[jeu.currentcolor].argile.get() + int(self.fenetretop.f6.argile.get()))
                jeu.listjoueurs[jeu.currentcolor].ble.set(
                    jeu.listjoueurs[jeu.currentcolor].ble.get() + int(self.fenetretop.f6.ble.get()))
                jeu.listjoueurs[jeu.currentcolor].laine.set(
                    jeu.listjoueurs[jeu.currentcolor].laine.get() + int(self.fenetretop.f6.laine.get()))
                jeu.listjoueurs[jeu.currentcolor].minerai.set(
                    jeu.listjoueurs[jeu.currentcolor].minerai.get() + int(self.fenetretop.f6.minerai.get()))

    def vert(self):
        if "Vert" in jeu.listjoueurs:
            if jeu.listjoueurs["Vert"].bois.get() >= int(self.fenetretop.f6.bois.get()) and jeu.listjoueurs[
                "Vert"].argile.get() >= int(self.fenetretop.f6.argile.get()) and jeu.listjoueurs[
                "Vert"].ble.get() >= int(self.fenetretop.f6.ble.get()) and jeu.listjoueurs["Vert"].laine.get() >= int(
                    self.fenetretop.f6.laine.get()) and jeu.listjoueurs["Vert"].minerai.get() >= int(
                    self.fenetretop.f6.minerai.get()):
                jeu.listjoueurs["Vert"].bois.set(
                    jeu.listjoueurs["Vert"].bois.get() - int(self.fenetretop.f6.bois.get()))
                jeu.listjoueurs["Vert"].argile.set(
                    jeu.listjoueurs["Vert"].argile.get() - int(self.fenetretop.f6.argile.get()))
                jeu.listjoueurs["Vert"].ble.set(jeu.listjoueurs["Vert"].ble.get() - int(self.fenetretop.f6.ble.get()))
                jeu.listjoueurs["Vert"].laine.set(
                    jeu.listjoueurs["Vert"].laine.get() - int(self.fenetretop.f6.laine.get()))
                jeu.listjoueurs["Vert"].minerai.set(
                    jeu.listjoueurs["Vert"].minerai.get() - int(self.fenetretop.f6.minerai.get()))
                jeu.listjoueurs[jeu.currentcolor].bois.set(
                    jeu.listjoueurs[jeu.currentcolor].bois.get() + int(self.fenetretop.f6.bois.get()))
                jeu.listjoueurs[jeu.currentcolor].argile.set(
                    jeu.listjoueurs[jeu.currentcolor].argile.get() + int(self.fenetretop.f6.argile.get()))
    ...
    def bleu(self):
        if "Bleu" in jeu.listjoueurs:
            if jeu.listjoueurs["Bleu"].bois.get() >= int(self.fenetretop.f6.bois.get()) and jeu.listjoueurs[
                "Bleu"].argile.get() >= int(self.fenetretop.f6.argile.get()) and jeu.listjoueurs[
                "Bleu"].ble.get() >= int(self.fenetretop.f6.ble.get()) and jeu.listjoueurs["Bleu"].laine.get() >= int(
                    self.fenetretop.f6.laine.get()) and jeu.listjoueurs["Bleu"].minerai.get() >= int(
                    self.fenetretop.f6.minerai.get()):
                jeu.listjoueurs["Bleu"].bois.set(
                    jeu.listjoueurs["Bleu"].bois.get() - int(self.fenetretop.f6.bois.get()))
                jeu.listjoueurs["Bleu"].argile.set(
                    jeu.listjoueurs["Bleu"].argile.get() - int(self.fenetretop.f6.argile.get()))
                jeu.listjoueurs["Bleu"].ble.set(jeu.listjoueurs["Bleu"].ble.get() - int(self.fenetretop.f6.ble.get()))
                jeu.listjoueurs["Bleu"].laine.set(
                    jeu.listjoueurs["Bleu"].laine.get() - int(self.fenetretop.f6.laine.get()))
                jeu.listjoueurs["Bleu"].minerai.set(
                    jeu.listjoueurs["Bleu"].minerai.get() - int(self.fenetretop.f6.minerai.get()))
                jeu.listjoueurs[jeu.currentcolor].bois.set(
                    jeu.listjoueurs[jeu.currentcolor].bois.get() + int(self.fenetretop.f6.bois.get()))
                jeu.listjoueurs[jeu.currentcolor].argile.set(
                    jeu.listjoueurs[jeu.currentcolor].argile.get() + int(self.fenetretop.f6.argile.get()))
    ...
    def orange(self):
        if "Orange" in jeu.listjoueurs:
            if jeu.listjoueurs["Orange"].bois.get() >= int(self.fenetretop.f6.bois.get()) and jeu.listjoueurs[
                "Orange"].argile.get() >= int(self.fenetretop.f6.argile.get()) and jeu.listjoueurs[
                "Orange"].ble.get() >= int(self.fenetretop.f6.ble.get()) and jeu.listjoueurs[
                "Orange"].laine.get() >= int(self.fenetretop.f6.laine.get()) and jeu.listjoueurs[
                "Orange"].minerai.get() >= int(self.fenetretop.f6.minerai.get()):
                jeu.listjoueurs["Orange"].bois.set(
                    jeu.listjoueurs["Orange"].bois.get() - int(self.fenetretop.f6.bois.get()))
                jeu.listjoueurs["Orange"].argile.set(
                    jeu.listjoueurs["Orange"].argile.get() - int(self.fenetretop.f6.argile.get()))
                jeu.listjoueurs["Orange"].ble.set(
                    jeu.listjoueurs["Orange"].ble.get() - int(self.fenetretop.f6.ble.get()))
                jeu.listjoueurs["Orange"].laine.set(
                    jeu.listjoueurs["Orange"].laine.get() - int(self.fenetretop.f6.laine.get()))
                jeu.listjoueurs["Orange"].minerai.set(
                    jeu.listjoueurs["Orange"].minerai.get() - int(self.fenetretop.f6.minerai.get()))
                jeu.listjoueurs[jeu.currentcolor].bois.set(
                    jeu.listjoueurs[jeu.currentcolor].bois.get() + int(self.fenetretop.f6.bois.get()))
                jeu.listjoueurs[jeu.currentcolor].argile.set(
                    jeu.listjoueurs[jeu.currentcolor].argile.get() + int(self.fenetretop.f6.argile.get()))
    ...
    def jaune(self):
        if "Jaune" in jeu.listjoueurs:
            if jeu.listjoueurs["Jaune"].bois.get() >= int(self.fenetretop.f6.bois.get()) and jeu.listjoueurs[
                "Jaune"].argile.get() >= int(self.fenetretop.f6.argile.get()) and jeu.listjoueurs[
                "Jaune"].ble.get() >= int(self.fenetretop.f6.ble.get()) and jeu.listjoueurs["Jaune"].laine.get() >= int(
                    self.fenetretop.f6.laine.get()) and jeu.listjoueurs["Jaune"].minerai.get() >= int(
                    self.fenetretop.f6.minerai.get()):
                jeu.listjoueurs["Jaune"].bois.set(
                    jeu.listjoueurs["Jaune"].bois.get() - int(self.fenetretop.f6.bois.get()))
                jeu.listjoueurs["Jaune"].argile.set(
                    jeu.listjoueurs["Jaune"].argile.get() - int(self.fenetretop.f6.argile.get()))
                jeu.listjoueurs["Jaune"].ble.set(jeu.listjoueurs["Jaune"].ble.get() - int(self.fenetretop.f6.ble.get()))
                jeu.listjoueurs["Jaune"].laine.set(
                    jeu.listjoueurs["Jaune"].laine.get() - int(self.fenetretop.f6.laine.get()))
                jeu.listjoueurs["Jaune"].minerai.set(
                    jeu.listjoueurs["Jaune"].minerai.get() - int(self.fenetretop.f6.minerai.get()))
                jeu.listjoueurs[jeu.currentcolor].bois.set(
                    jeu.listjoueurs[jeu.currentcolor].bois.get() + int(self.fenetretop.f6.bois.get()))
                jeu.listjoueurs[jeu.currentcolor].argile.set(
                    jeu.listjoueurs[jeu.currentcolor].argile.get() + int(self.fenetretop.f6.argile.get()))
...
class monop:  # carte monopole
    def __init__(self, _root):
        ...
        self.fenetretop.bois = Button(self.fenetretop, text="Bois",
                                      command=(lambda: jeu.monop(self.fenetretop, "bois")), width=20)
        self.fenetretop.bois.grid()
        self.fenetretop.bois = Button(self.fenetretop, text="Argile",
                                      command=(lambda: jeu.monop(self.fenetretop, "argile")), width=20)
        self.fenetretop.bois.grid()
        self.fenetretop.bois = Button(self.fenetretop, text="Ble", command=(lambda: jeu.monop(self.fenetretop, "ble")),
                                      width=20)
        self.fenetretop.bois.grid()
        self.fenetretop.bois = Button(self.fenetretop, text="Laine",
                                      command=(lambda: jeu.monop(self.fenetretop, "laine")), width=20)
        self.fenetretop.bois.grid()
        self.fenetretop.bois = Button(self.fenetretop, text="Minerai",
                                      command=(lambda: jeu.monop(self.fenetretop, "minerai")), width=20)
        self.fenetretop.bois.grid()
...
class discov:  # carte decouverte
    def __init__(self, _root):
        ...
        self.fenetretop.bois.grid()
        self.fenetretop.bois = Button(self.fenetretop, text="2 Bois",
                                      command=(lambda: jeu.discov(self.fenetretop, "bois")), width=15)
        self.fenetretop.bois.grid()
        self.fenetretop.bois = Button(self.fenetretop, text="2 Argile",
                                      command=(lambda: jeu.discov(self.fenetretop, "argile")), width=15)
        self.fenetretop.bois.grid()
        self.fenetretop.bois = Button(self.fenetretop, text="2 Ble",
                                      command=(lambda: jeu.discov(self.fenetretop, "ble")), width=15)
        self.fenetretop.bois.grid()
        self.fenetretop.bois = Button(self.fenetretop, text="2 Laine",
                                      command=(lambda: jeu.discov(self.fenetretop, "laine")), width=15)
        self.fenetretop.bois.grid()
        self.fenetretop.bois = Button(self.fenetretop, text="2 Minerai",
                                      command=(lambda: jeu.discov(self.fenetretop, "minerai")), width=15)
        self.fenetretop.bois.grid()
...
class cartes:  # menu des cartes (mis dans "menubar")
    def __init__(self, _gestionnaireFenetres, _root):
        ...
        if jeu.listjoueurs[jeu.currentcolor].cards[0] > 0:
            self.fenetretop.victoire = Button(self.fenetretop, text=(
                        "Reveler Carte Victoire (" + str(jeu.listjoueurs[jeu.currentcolor].cards[0]) + ")"),
                                              command=self.victory, width=30)
            self.fenetretop.victoire.grid()
        if jeu.listjoueurs[jeu.currentcolor].cards[1] > 0:
            self.fenetretop.chevalier = Button(self.fenetretop, text=(
                        "Reveler Carte Chevalier (" + str(jeu.listjoueurs[jeu.currentcolor].cards[1]) + ")"),
                                               command=(lambda: self.knight(_gestionnaireFenetres)), width=30)
            self.fenetretop.chevalier.grid()
        if jeu.listjoueurs[jeu.currentcolor].cards[2] > 0:
            self.fenetretop.monopole = Button(self.fenetretop, text=(
                        "Reveler Carte Monopole (" + str(jeu.listjoueurs[jeu.currentcolor].cards[2]) + ")"),
                                              command=self.monop, width=30)
            self.fenetretop.monopole.grid()
        if jeu.listjoueurs[jeu.currentcolor].cards[3] > 0:
            self.fenetretop.decouvr = Button(self.fenetretop, text=(
                        "Reveler Carte Decouverte (" + str(jeu.listjoueurs[jeu.currentcolor].cards[3]) + ")"),
                                             command=self.discovery, width=30)
