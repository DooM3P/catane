...
class Jeu:
    def __init__(self, _ile):
        ...
        self.cards = ["Chevalier", "Chevalier", "Chevalier", "Chevalier", "Chevalier", "Chevalier", "Chevalier",
                      "Chevalier", "Chevalier", "Chevalier", "Chevalier", "Chevalier", "Chevalier", "Chevalier",
                      "Victoire", "Victoire", "Victoire", "Victoire", "Victoire", "Monopole", "Decouverte",
                      "Decouverte", "Decouverte", "Monopole", "Monopole"]
    ...
    def transcolor(self,
                   _couleur):  # attribue au joueur, dont la couleur est en francais, sa couleur "informatique" (utile pour les routes)
        color = "grey"
    ...
    def findplyr(self, _listjoueurs, _numde):  # trouve le joueur dont c'est le de dans "order"
        for i in list(_listjoueurs.keys()):
    ...
    def compteur(self, _jeu, _gestionfenetres):  # le decompte des points
        for i in list(_jeu.listjoueurs.keys()):
            compteur = 0
            for j in list(_jeu.listjoueurs[i].villages.keys()):
    ...
    def monop(self, _fenetre, _matiere):  # la carte monopole donne toutes les matieres d'un type au joueur qui la joue
        compteur = 0
        if _matiere == "bois":
            for i in list(self.listjoueurs.keys()):
...
class Ile:  # contient des fonctions de l'interface de jeu comme de creation du plateau
    ...
    def somtest2(self, _num, _currentcolor, _jeu):  # le sommet a t-il une route qui y mene ?
        for i in _jeu.ile.sommets[_num].listsuccess:
            if (i.num, _num) in _jeu.ile.routes and _jeu.ile.routes[(i.num, _num)].color == _currentcolor:
                return 1
            if (_num, i.num) in _jeu.ile.routes and _jeu.ile.routes[(_num, i.num)].color == _currentcolor:
                return 1
        return 0

    def routetest(self, _bout1, _bout2, _currentcolor,
                  _jeu):  # y a t-il une route ou une colonie a l'un de ces deux bouts ?
        if _jeu.ile.sommets[_bout1].constr[1] == _currentcolor:
            return 1
        else:
            for i in _jeu.ile.sommets[_bout1].listsuccess:
                if (i.num, _bout1) in _jeu.ile.routes and _jeu.ile.routes[(i.num, _bout1)].color == _currentcolor:
                    return 1
                if (_bout1, i.num) in _jeu.ile.routes and _jeu.ile.routes[(_bout1, i.num)].color == _currentcolor:
                    return 1
        if _jeu.ile.sommets[_bout2].constr[1] == _currentcolor:
            return 1
        else:
            for i in _jeu.ile.sommets[_bout2].listsuccess:
                if (i.num, _bout2) in _jeu.ile.routes and _jeu.ile.routes[(i.num, _bout2)].color == _currentcolor:
                    return 1
                if (_bout2, i.num) in _jeu.ile.routes and _jeu.ile.routes[(_bout2, i.num)].color == _currentcolor:
                    return 1
        return 0

    def affiche(self):  # fonction qui affichait les hexagones crees (pour le deboguage)
        listeHexagones = list(self.hexacoord.values())
        for hexagone in listeHexagones: print(hexagone)
        for sommet in list(self.sommets.values()):
            chaine = ""
    ...
    def ajouterhexagone(self, hexagone):
        ...
        if hexagone.numde in self.hexanumde:  # creation de la liste hexanumde avec ajout a une cle deja existante ou creation de la cle
            self.hexanumde[hexagone.numde].append(hexagone)
        else:
            self.hexanumde[hexagone.numde] = [hexagone]

    def hasHexagoneHautGauche(self, x, y):
        return (x - self.taille / 2, y + (3 ** (0.5)) * self.taille / 2) in self.hexacoord

    def getHexagoneHautGauche(self, x, y):
        if (x - self.taille / 2, y + (3 ** (0.5)) * self.taille / 2) in self.hexacoord:
    ...
    def hasHexagoneGauche(self, x, y):
        return (x - self.taille, y) in self.hexacoord

    def getHexagoneGauche(self, x, y):
        if (x - self.taille, y) in self.hexacoord:
    ...
    def hasVoisin(self, x, y):
        return self.hasHexagoneHautGauche(x, y) or self.hasHexagoneHautDroite(x, y) or self.hasHexagoneGauche(x,
                                                                                                              y) or self.hasHexagoneDroite(
            x, y) or self.hasHexagoneBasGauche(x, y) or self.hasHexagoneBasDroite(x, y)
...
class Hexagone:
    ...
    def __init__(self, _ile, _x, _y, _matiere, _numde):
        ...
        if self.ile.hasHexagoneHautGauche(self.x,
                                          self.y):  # on renumerote les sommets qui sont "partages" avec un hexagone deja existant et adjacents en leur donnant les "anciens" numero ; il y a des numero de sommet qui "sautent" car supprimes comme etant redondants
            ...
            if self.ile.nbhexaposes * 6 + 1 in self.ile.sommets:
            ...
            if self.ile.nbhexaposes * 6 + 6 in self.ile.sommets:
        ...
        if self.ile.hasHexagoneHautDroite(self.x, self.y):
            ...
            if self.ile.nbhexaposes * 6 + 2 in self.ile.sommets:
        ...
        if self.ile.hasHexagoneGauche(self.x, self.y):
            ...
            if self.ile.nbhexaposes * 6 + 5 in self.ile.sommets:
        ...
        if self.ile.hasHexagoneDroite(self.x, self.y):
            ...
            if self.ile.nbhexaposes * 6 + 3 in self.ile.sommets:
        ...
        if self.ile.hasHexagoneBasGauche(self.x, self.y):
            ...
            if self.ile.nbhexaposes * 6 + 4 in self.ile.sommets:
        ...
        for i in list(self.ile.sommets.keys()):
